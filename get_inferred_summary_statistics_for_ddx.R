...
sddx <- function(ldsc_intercept, gwas1, t1.case, gwas2, t2.case){
  ...
  if ('eaf' %in% colnames(gwas1)&'eaf' %in% colnames(gwas2)) {
  ddx.eaf=(gwas1$eaf*gwas1$n+gwas2$eaf*gwas2$n)/(gwas1$n+gwas2$n)  
  
  attach(gwas1)
  ddx.gwas <- data.frame(beta=ddx_beta,se=ddx_se,pval=ddx_pval,snpid=snpid,chr=chr,bpos=bpos,a1=a1,a2=a2,n=ddx.n,eaf=ddx.eaf) 
  detach(gwas1)} else {
  
  cat('there is no eaf column in gwas1 or gwas2',sep='\n')
  attach(gwas1)
  ddx.gwas <- data.frame(beta=ddx_beta,se=ddx_se,pval=ddx_pval,snpid=snpid,chr=chr,bpos=bpos,a1=a1,a2=a2,n=ddx.n) 
  detach(gwas1)
  }
  return(ddx.gwas)
}
